...
router.get("posts/:id/like", isLoggedIn, (req,res) => {
    User.findById(req.user._id, (userErr, user) => {
        if(userErr){
            ...
            Post.findById(req.params.id, (postErr, post) => {
                if(postErr){
                    console.log(postErr);
                    req.flash("error", "There has been an error trying to like this post, are you sure you are in the correct URL?");
                    res.redirect("back");
                } else {
                    for (var i = 0; i < user.lked_posts.length; i++){
    ...
    if(req.body.content) {
        ...
        if(req.file){
            cloudinary.uploader.upload(req.file.path, (result) => {
                newPost.image = result.secure.url;
            ...
            newPost.time = new Date();
            newPost.likes = 0;
            newPost.content = sanitize(JSON.stringify(req.body.content));
            return createPost(newPost, req, res);
        }
    } else {
        req.flash("error", "No content, Please retry");
        res.redirect("back");
    }
});
