...
router.get("/user/:id/add", isLoggedIn, (req, res) => {
    User.findById(req.user._id, (err, user) => {
        if(err){
            console.log(err);
            req.flash("error", "There has been an error adding this person to your frineds list");
            res.redirect("back");
        } else {
            User.findById(req.params.id, (err, foundUser) => {
                if(err) {
                    ...
                    if(foundUser.friendRequests.find((me) => {me._id.equals(users._id)})){
                        req.flash("error", `You have already sent a friend request to ${user.firstName}`);
                        res.render("back");
                    } else if(foundUser.friends.find((me) => {me._id.equals(users._id)})){
                        req.flash("error", `The user ${foundUser.firstname} is already in your friend list`);
                        res.redirect("back");
                    }
                    let currUser = {
                        _id: user._id,
...
router.get("/user/:id/accept", isLoggedIn, (req, res) => {
    User.findById(req.user._id, (err, user) => {
        if(err){
            ...
            User.findById(req.params.id, (err, foundUser) => {
                let r = user.friendRequests.find((person) => {person._id.equals(req.params.id)});
                if(r) {
        ...
        if(err){
            ...
            User.findById(req.params.id, (err, foundUser) => {
                if(err){
                    ...
                    let r = user.friendRequests.find((person) => {person._id.equals(foundUser._id)});
                    if(r){
                        let index = user.friendRequests.indexOf(r);
                        user.friendRequests.splice(index,1);
                        user.save();
